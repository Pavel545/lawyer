...
export default function Services() {
  ...
  return (
    <>
      ...
      <SEO
        title={replaceTemplate(current.meta?.title) || current.name}
        description={replaceTemplate(current.meta?.description) || `Услуги ${current.name} в ${replaceTemplate('{{city.in}}')}`}
        keywords={replaceTemplate(current.meta?.keywords) || `${current.name}, услуги ${current.name}, ${replaceTemplate('{{city.in}}')}, ${replaceTemplate('{{city.in}}')} услуги ${current.name}`}
      />
      <main className="services">
        <BreadCrumbs />
        <section className="services_faise">
          <div className="container flex">
            <h1 className="h2">{replaceTemplate(current.name)}</h1>
              ...
              <div className="services_faise_text">
                {current.text && (
                  <p
                    ...
                    dangerouslySetInnerHTML={{ __html: replaceTemplate(current.text) }}
                  />
                )}
                  ...
                  {Array.isArray(current.list) && current.list.length > 0 && (
                    <ul className="services_faise_list">
                      {current.list.map((item, idx) => (
                        <li key={`${current.title}-li-${idx}`}>{replaceTemplate(item)}</li>
                      ))}
                    </ul>
                  )}
                </div>
                ...
                {current.text2 && (
                  <p
                    className="services_faise_text"
                    dangerouslySetInnerHTML={{ __html: replaceTemplate(current.text2) }}
                  />
                )}
                ...
                {current.textEnd && (
                  <p
                    className="services_faise_text"
                    dangerouslySetInnerHTML={{ __html: replaceTemplate(current.textEnd) }}
                  />
                )}
              </div>
            </div>
